...
def find_similar_jobs_vector(resume_embedding: list, embedding_type: str = 'word2vec', top_k: int = 10) -> list:
    """Find top-k similar jobs using vector similarity search. Defaults to Word2Vec (300 dim)."""
    engine = create_db_engine()
    if engine is None:
        return []
    ...
    try:
        with engine.connect() as conn:
            # Choose embedding column based on type (default to word2vec)
            if embedding_type in ('word2vec', 'w2v'):
                embedding_column = 'word2vec_embedding'
            else:
                embedding_column = 'embedding'  # SBERT
