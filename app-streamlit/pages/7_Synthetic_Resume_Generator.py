...
st.title("🧪 Synthetic Resume Generator (Ollama)")
...
st.markdown(
    """
### Overview
Create a **synthetic, realistic resume** using a local **Ollama** model in two stages:
...
st.markdown("---")
st.markdown("### 1️⃣ Define Target Profile")
...
st.markdown("---")
st.markdown("### 2️⃣ Batch Generation Settings")
...
with col_gen2:
    regenerate_resume_only = st.button(
        "🔁 Regenerate Resume from Last Profile",
        use_container_width=True,
    )
...
if generate_both:
    if max_resumes == 1 and not use_random_params:
        ...
        if not job_title.strip():
            ...
            with st.spinner("Generating candidate profile with Ollama..."):
                profile = generate_candidate_profile(
                    model=model_name.strip(),
                    job_title=job_title.strip(),
                    years_experience=int(years_experience),
...
if profile is not None:
    st.markdown("---")
    st.markdown("### 2️⃣ Generated Candidate Profile")
...
if resume is not None:
    st.markdown("---")
    st.markdown("### 3️⃣ Generated Resume")
    st.text_area(
