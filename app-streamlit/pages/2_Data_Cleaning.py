...
def build_job_text(row):
    """Build combined job text from multiple columns (matches notebook)"""
    parts = []
    ...
    # Note: Experience is not included (matches notebook behavior)
...
if st.session_state.raw_jobs_df is not None:
    ...
    with col2:
        ...
        deduplicate_on_text = st.checkbox("Deduplicate on cleaned text", value=True, help="Remove duplicates based on cleaned text only")
    ...
    if st.button("Apply Cleaning", type="primary"):
        with st.spinner("Cleaning data..."):
            ...
            # Deduplicate on cleaned text (matches notebook behavior)
            if deduplicate_on_text:
                if 'job_text_cleaned' in cleaned_df.columns:
                    ...
                    st.info(f"Deduplicated based on cleaned text: reduced from {initial_count:,} to {len(cleaned_df):,} unique jobs")
                else:
                    st.warning("Cannot deduplicate: 'job_text_cleaned' column not found. Enable 'Apply text cleaning' first.")
...
if st.session_state.cleaned_jobs_df is not None:
    ...
    with col3:
        raw_df = st.session_state.get('raw_jobs_df')
